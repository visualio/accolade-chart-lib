...
function peg$parse(input, options) {
  ...
  var peg$FAILED = {}, peg$startRuleFunctions = { svg_path: peg$parsesvg_path }, peg$startRuleFunction = peg$parsesvg_path, peg$c0 = function(data) {
    ...
    return digits.join("");
  }, peg$c50 = /^[ \t\n\r]/, peg$c51 = peg$classExpectation([" ", "	", "\n", "\r"], false, false), peg$currPos = 0, peg$posDetailsCache = [{ line: 1, column: 1 }], peg$maxFailPos = 0, peg$maxFailExpected = [], peg$result;
...
var reIsNative$1 = RegExp("^" + fnToString$1.call(hasOwnProperty$3).replace(/[\\^$.*+?()[\]{}|]/g, "\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") + "$");
function getNative$2(object2, key) {
  var value = object2 == null ? void 0 : object2[key];
  return isNative$1(value) ? value : void 0;
}
function isFunction$2(value) {
  return isObject$4(value) && objToString$1.call(value) == funcTag$2;
}
...
var argsTag = "[object Arguments]", funcTag$1 = "[object Function]", genTag = "[object GeneratorFunction]";
var objectProto$2 = Object.prototype;
var hasOwnProperty$2 = objectProto$2.hasOwnProperty;
var objectToString = objectProto$2.toString;
var propertyIsEnumerable = objectProto$2.propertyIsEnumerable;
function isArguments$1(value) {
  return isArrayLikeObject(value) && hasOwnProperty$2.call(value, "callee") && (!propertyIsEnumerable.call(value, "callee") || objectToString.call(value) == argsTag);
}
...
var objToString = objectProto$1.toString;
var reIsNative = RegExp("^" + fnToString.call(hasOwnProperty$1).replace(/[\\^$.*+?()[\]{}|]/g, "\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") + "$");
...
var isArray$1 = nativeIsArray || function(value) {
  return isObjectLike(value) && isLength$1(value.length) && objToString.call(value) == arrayTag;
};
function isFunction(value) {
  return isObject$2(value) && objToString.call(value) == funcTag;
}
...
var getNative = lodash__getnative, isArguments = lodash_isarguments, isArray = lodash_isarray;
var reIsUint = /^\d+$/;
...
function shimKeys(object2) {
  var props = keysIn(object2), propsLength = props.length, length = propsLength && object2.length;
  var allowIndexes = !!length && isLength(length) && (isArray(object2) || isArguments(object2));
  var index = -1, result = [];
  while (++index < propsLength) {
    var key = props[index];
    if (allowIndexes && isIndex(key, length) || hasOwnProperty.call(object2, key)) {
      result.push(key);
    }
  }
  return result;
...
var keys$1 = !nativeKeys ? shimKeys : function(object2) {
  var Ctor = object2 == null ? void 0 : object2.constructor;
  if (typeof Ctor == "function" && Ctor.prototype === object2 || typeof object2 != "function" && isArrayLike(object2)) {
    return shimKeys(object2);
  }
  return isObject$1(object2) ? nativeKeys(object2) : [];
};
function keysIn(object2) {
  ...
  var length = object2.length;
  length = length && isLength(length) && (isArray(object2) || isArguments(object2)) && length || 0;
  var Ctor = object2.constructor, index = -1, isProto = typeof Ctor == "function" && Ctor.prototype === object2, result = Array(length), skipIndexes = length > 0;
  while (++index < length) {
    result[index] = index + "";
  }
  for (var key in object2) {
    if (!(skipIndexes && isIndex(key, length)) && !(key == "constructor" && (isProto || !hasOwnProperty.call(object2, key)))) {
      result.push(key);
    }
  }
  return result;
}
var lodash_keys = keys$1;
var keys = lodash_keys;
...
function pairs(object2) {
  object2 = toObject(object2);
  var index = -1, props = keys(object2), length = props.length, result = Array(length);
  while (++index < length) {
    var key = props[index];
    result[index] = [key, object2[key]];
  }
  return result;
}
...
function axis(orient, scale) {
  ...
  function axis2(context) {
    var values = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain() : tickValues, format2 = tickFormat2 == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments) : identity$4 : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range2 = scale.range(), range0 = +range2[0] + offset2, range1 = +range2[range2.length - 1] + offset2, position = (scale.bandwidth ? center : number$1)(scale.copy(), offset2), selection2 = context.selection ? context.selection() : context, path2 = selection2.selectAll(".domain").data([null]), tick = selection2.selectAll(".tick").data(values, scale).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
...
function arc() {
  ...
  function arc2() {
    ...
    if (!(r1 > epsilon))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da, da1 = da, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon && (padRadius ? +padRadius.apply(this, arguments) : sqrt(r0 * r0 + r1 * r1)), rc = min$2(abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t0, t1;
      if (rp > epsilon) {
...
function areaChart(x0, y0, y1) {
  ...
  function arealine() {
    return lineChart().defined(defined).curve(curve).context(context);
  }
  area.x = function(_) {
...
function redrawBarChart({ chart, set: set2, xScale, yScale, colors: colors$1, cols, labels, width, height, unit: unit2 }) {
  ...
  createLabels(chart, labels, width, height, getMaxLength(set2.map(([, val]) => ({ values: [val] }))));
  const areaGenerator = areaChart().curve(curveLinear).x(([key]) => xScale(key) + xScale.bandwidth() / 3 * 2).y1(([, value]) => yScale(value)).y0(() => height);
  const area = chart.selectAll(`.area`).data([1]);
  area.exit().remove();
  const areaEnter = area.enter().append(`path`).attr(`class`, `area`).attr(`fill`, colors.area).attr(`opacity`, 0.5);
  areaEnter.merge(area).transition(t).delay(2e3).attrTween(`d`, () => {
    const interpolator = interpolateArray(set2.map(() => 0), set2.map(([, val]) => val));
    return (trans) => areaGenerator(interpolator(trans).map((value, index) => [set2[index][0], value]));
  });
  const bars = chart.selectAll(`.bar`).data(set2, ([key]) => key);
  bars.exit().remove();
  const barsEnter = bars.enter().append(`g`).attr(`font-family`, fontFamilyPrimary).attr(`class`, `bar`);
  bars.merge(barsEnter);
  const getBarPath = ([key, value], init2) => createBarPath(xScale(key), init2 === `init` ? height : yScale(value), xScale.bandwidth(), height);
  ...
  barsEnter.merge(bars.select(`.bar__color`).attr(`fill`, getBarFill).transition(t).attr(`d`, getBarPath));
  const lineGenerator = lineChart().curve(curveLinear).x(([key]) => xScale(key) + xScale.bandwidth() / 3 * 2).y(([, value]) => yScale(value));
  const line = chart.selectAll(`.line`).data([1]);
  line.exit().remove();
  const lineEnter = line.enter().append(`path`).attr(`class`, `line`).attr(`fill`, `none`).attr(`stroke`, colors.line).attr(`stroke-linejoin`, `round`).attr(`stroke-linecap`, `round`).attr(`stroke-width`, 2);
  lineEnter.merge(line).transition(t).delay(2e3).attrTween(`d`, () => {
    const interpolator = interpolateArray(set2.map(() => 0), set2.map(([, val]) => val));
    return (trans) => lineGenerator(interpolator(trans).map((value, index) => {
      return [set2[index][0], value];
    }));
  });
  const getSymbolPath = ([key, value], init2) => createBarSymbol(xScale(key), init2 === `init` ? height : yScale(value), xScale.bandwidth());
  const getSymbolFill = ([key], opacity = false) => {
    const colorKey = cols[key].colors;
    const { border: { r, g, b, a } } = colors$1[colorKey];
    return opacity === true ? a : `rgb(${r}, ${g}, ${b})`;
  };
  barsEnter.append(`path`).attr(`class`, `bar__symbol`).attr(`fill`, (pair) => getSymbolFill(pair)).attr(`fill-opacity`, (pair) => getSymbolFill(pair, true)).attr(`d`, (d) => getSymbolPath(d, `init`)).transition(t).delay(delay).attr(`d`, getSymbolPath).attr(`fill`, getSymbolFill);
  barsEnter.merge(bars.select(`.bar__symbol`).transition(t).attr(`d`, getSymbolPath).attr(`fill`, getSymbolFill));
  ...
  barsEnter.append(`text`).attr(`class`, `bar__text`).attr(`fill`, `white`).attr(`text-anchor`, `middle`).attr(`font-weight`, `bold`).text((current) => createTextObject(current).value).attr(`font-size`, getTextFontSize).attr(`letter-spacing`, getTextLetterSpacing).attr(`font-family`, fontFamilyPrimary).attr(`x`, getTextX).attr(`y`, () => getTextY([])).transition(t).delay(delay).attr(`y`, getTextY);
  barsEnter.merge(bars.select(`.bar__text`).text((current) => createTextObject(current).value).transition(t).attr(`font-size`, getTextFontSize).attr(`x`, getTextX).attr(`y`, getTextY));
  const getUnitFontSize = () => xScale.bandwidth() / 4;
  const getUnitX = ([key]) => xScale(key) + xScale.bandwidth() / 3 * 2;
  const getUnitY = ([, value], init2) => (init2 === `init` ? height : yScale(value)) - xScale.bandwidth() * symbolIndentRatio + getUnitFontSize() / 5 * 3;
  barsEnter.append(`text`).attr(`class`, `bar__units`).attr(`fill`, `white`).attr(`text-anchor`, `middle`).attr(`font-weight`, `bold`).text((current) => unit2 || createTextObject(current).unit).attr(`font-size`, getUnitFontSize).attr(`x`, getUnitX).attr(`y`, (pair) => getUnitY(pair, `init`)).transition(t).delay(delay).attr(`y`, getUnitY).attr(`font-size`, getUnitFontSize);
  barsEnter.merge(bars.select(`.bar__units`).text((current) => unit2 || createTextObject(current).unit).transition(t).attr(`x`, getUnitX).attr(`y`, getUnitY).attr(`font-size`, getUnitFontSize));
  ...
  const points = chart.selectAll(`.point`).data(set2, ([key]) => key);
  points.exit().remove();
...
function getLengthFromAxis(axis2) {
...
const arrow = (options) => ({
  ...
  async fn(middlewareArguments) {
    ...
    const axis2 = getMainAxisFromPlacement(placement);
    const alignment = getAlignment(placement);
    const length = getLengthFromAxis(axis2);
    ...
    let clientSize = arrowOffsetParent ? axis2 === "y" ? arrowOffsetParent.clientHeight || 0 : arrowOffsetParent.clientWidth || 0 : 0;
    if (clientSize === 0) {
    ...
    const offset2 = within(min2, center2, max2);
    const alignmentPadding = alignment === "start" ? paddingObject[minProp] : paddingObject[maxProp];
    const shouldAddOffset = alignmentPadding > 0 && center2 !== offset2 && rects.reference[length] <= rects.floating[length];
    const alignmentOffset = shouldAddOffset ? center2 < min2 ? min2 - center2 : max2 - center2 : 0;
    return {
...
function createDefs({ chart, colors: colorMap, t, set: set2, xScale, idPrefix }) {
  ...
  gradientsEnter.append(`stop`).attr(`class`, `gradient__start`).attr(`offset`, `20%`).attr(`stop-color`, getStartColor).attr(`stop-opacity`, getStartOpacity).transition(t).attr(`stop-color`, getStartColor).attr(`stop-opacity`, getStartOpacity);
  ...
  gradientsEnter.append(`stop`).attr(`class`, `gradient__end`).attr(`offset`, `100%`).attr(`stop-color`, getEndColor).attr(`stop-opacity`, getEndOpacity).transition(t).attr(`stop-color`, getEndColor).attr(`stop-opacity`, getEndOpacity);
  ...
  clipsEnter.append(`rect`).attr(`class`, `clip__shape`).attr(`width`, getClipWidth).attr(`x`, getClipX).attr(`height`, 5e3).attr(`y`, 0).transition(t).attr(`width`, getClipWidth).attr(`x`, getClipX);
...
function getClipDimensions(values, xScale) {
  ...
  const set2 = findLongestConsecutive(Object.keys(obj).map(parseFloat));
  const [first, last] = [set2[0], set2[set2.length - 1]];
  return set2.length ? {
...
function redrawLineChart({ chart, set: set2, xScale, yScale, colors: colors$1, labels, height, width, locale: locale2, cols, decimalPlaces }) {
  ...
  const topLabelsEnter = topLabels.enter().append(`tspan`).attr(`class`, `topLabels`);
  ...
  const masterLine = lineChart().curve(monotoneX).x(([key]) => xScale(key) + xScale.bandwidth() / 2).y(([, value]) => yScale(value));
  const slaveLine = lineChart().curve(monotoneX).x(([key]) => xScale(key) + xScale.bandwidth() / 2).y(([key, value]) => value ? yScale(value) : yScale(master.values[key]));
  const lines = chart.selectAll(`.line`).data(set2.sort(([key]) => isMaster(key) ? 1 : -1), ([key]) => key);
  lines.exit().remove();
  const linesEnter = lines.enter().append(`path`).attr(`class`, `line`).attr(`fill`, `none`).attr(`stroke-linejoin`, `round`).attr(`stroke-linecap`, `round`).attr(`stroke-width`, 2);
  linesEnter.merge(lines).attr(`clip-path`, ([key]) => !isMaster(key) && `url(#clip${key})`).transition(t).attr(`stroke`, ([, { colors: colorKey }]) => {
    const { border: { r, g, b } } = colors$1[colorKey];
    return `rgb(${r}, ${g}, ${b})`;
  }).attrTween(`d`, ([key, { values: map2 }]) => {
    const values = lodash_pairs(map2);
  ...
  chart.selectAll(`.annotation-area`).data(annotationData).enter().append(`rect`).attr(`class`, `annotation-area`).attr(`fill`, "transparent").attr(`x`, ([key]) => xScale(key) + xScale.bandwidth() / 2 - 14).attr(`y`, ([, , value]) => yScale(value) - 14).attr(`width`, 28).attr(`height`, ([, , value]) => height - yScale(value) + 50).on("mouseover", (x2, [key, [head, ...rest]]) => tooltip.style("opacity", "1").html(`
    ...
    }).then(({ x: x2, y: y2, middlewareData: { arrow: arrow2 }, placement }) => {
      Object.assign(floatingElement.style, {
        top: placement === "top" ? `${y2}px` : 0,
...
function redrawDoughnutChart({ chart, set: set2, colors: colors2, cols, width, sortDirection, sortType }) {
  ...
  const data = pie().value(([, value]) => value).sort(null)(set2.sort(sortData(sortType, sortDirection, cols)).map((val) => val));
  const radius = width / 2;
  const innerRadius = radius * 0.55;
  const pieGen = arc().innerRadius(innerRadius).outerRadius(radius);
  const arcTween = (newData, idx) => {
    const initData = __spreadProps(__spreadValues({}, newData), {
    ...
    const interpolator = interpolate$1(initData, newData);
    return function(tr) {
      const sth = interpolator(tr);
      return pieGen(sth, idx);
    };
  };
  const doughnut = chart.selectAll(`.doughnut`).data([1]).enter().append(`g`).attr(`class`, `doughnut`).attr(`transform`, `translate(${width / 2}, ${width / 2})`);
  ...
  arcsEnter.merge(arcs).transition(t).attrTween(`d`, arcTween).attr(`fill`, (d, idx) => {
    const [key] = set2[idx];
    const { colors: colorKey } = cols[key];
    const { border: { r, g, b } } = colors2[colorKey];
    return `rgb(${r}, ${g}, ${b})`;
  });
  const center2 = doughnut.selectAll(`.center`).data([1]);
  const centerEnter = center2.enter().append(`circle`).attr(`class`, `center`).attr(`stroke`, `white`).attr(`stroke-width`, 60).attr(`stroke-opacity`, 0.8).attr(`cx`, 0).attr(`cy`, 0).attr(`fill`, `none`).attr(`r`, 0);
  centerEnter.merge(center2).transition().duration(500).attr(`r`, innerRadius);
}
function updateTable(table, { cols: c, rows: r, colors: colors2, isTransposed }) {
  const [cols, rows] = isTransposed ? transposeTable(c, r) : [c, r];
  emptyTable(table);
  createTableHeader(table, cols);
  createTableBody(table, rows, cols, colors2);
}
function transposeTable(cols, rows) {
  const [[firstColId, firstColValue], ...restCols] = Object.entries(cols);
  const transposedCols = __spreadValues({
    0: firstColValue
  }, Object.fromEntries(Object.entries(rows).map(([key, { values, colors: colors2 }]) => [key, { colors: colors2, value: values[firstColId] }])));
  const transposedRows = Object.fromEntries(restCols.map(([key, { value, colors: colors2 }]) => [key, {
    colors: colors2,
    values: __spreadValues({
      0: value
    }, Object.fromEntries(Object.entries(rows).map(([k, { values }]) => [k, values[key]])))
  }]));
...
function createTableHeader(table, cols) {
  ...
  for (const colKey in cols) {
    ...
    row.appendChild(col);
    colIterator === 1 && row.appendChild(col.cloneNode(true));
  }
  head.appendChild(row);
  table.appendChild(head);
}
function createTableBody(table, rows, cols, colors2) {
  const body = document.createElement(`tbody`);
  const styleCell = makeTableCellStyle(colors2);
...
function makeTableCellStyle(colors2) {
  if (!colors2)
    return () => null;
  return (cell, rowColorKey, colColorKey, isEven) => {
    const { start: rowBgColor, border: rowTxtColor } = colors2[rowColorKey] || transparentColor;
    const { start: oddColor, end: evenColor, border: colTxtColor } = colors2[colColorKey] || transparentColor;
    if (rowBgColor.a)
...
function getChartHeight(state) {
  return state.height - getChartBottomMargin(state);
}
function getChartLeftMargin({ rows, chartType, verticalLabel }) {
  if (isDoughnutChart(chartType))
    return 0;
  return getMaxLength(rows) * fontSize * fontAspectRatio + (verticalLabel ? labelOffset : 0);
}
function getChartTopMargin({ chartType }) {
  if (isDoughnutChart(chartType))
    return 0;
  return margin.top;
}
function getChartBottomMargin({ chartType }) {
  if (isDoughnutChart(chartType))
    return 0;
  return margin.bottom + labelOffset;
}
function getChartSpecificData({ chartType, rows }, forScales = true) {
  if (isLineChart(chartType)) {
    if (!forScales)
      return lodash_pairs(rows);
    const maxValues = {};
    Object.keys(rows).forEach((rowKey2) => {
      Object.keys(rows[rowKey2].values).forEach((valueKey) => {
        maxValues[valueKey] = parseInt(maxValues[valueKey], 10) > rows[rowKey2].values[valueKey] ? maxValues[valueKey] : rows[rowKey2].values[valueKey];
      });
    });
    return lodash_pairs(maxValues);
  }
  const [rowKey] = Object.keys(rows);
  return lodash_pairs(rows[rowKey].values);
}
function prepareChart(wrapper, state) {
  const set2 = getChartSpecificData(state);
  const xAxis = createXAxis(set2, state);
  const yAxis = createYAxis(set2, state);
  const svg = wrapper.append(`svg`).attr(`viewBox`, `0 0 ${state.width} ${state.height}`).attr(`fontStyle`, `normal`).attr(`fontFamily`, fontFamilyPrimary);
  const chart = svg.append(`g`).attr(`transform`, `translate(${getChartLeftMargin(state)}, ${getChartTopMargin(state)})`);
  chart.append(`g`).attr(`class`, `axis axis--y`).call(yAxis);
  chart.append(`g`).attr(`class`, `axis axis--x`).attr(`transform`, `translate(0, ${getChartHeight(state)})`).call(xAxis);
  chart.append(`defs`);
  return chart;
}
function createXAxis(set2, state) {
  if (isDoughnutChart(state.chartType))
    return () => null;
  const scale = createXScale(set2, state);
  return (g) => {
    const axis2 = axisBottom(scale).tickSize(0);
    g.call(axis2);
    g.select(`.domain`).remove();
    const text = g.selectAll(`.tick text`).attr(`font-size`, fontSize).attr(`font-family`, fontFamilyPrimary).attr(`dy`, 16).attr(`fill`, colors.label).text(``);
    text.append(`tspan`).attr(`dy`, `1.4em`).attr(`x`, `0`).text((key) => getLegendLine(state.cols[key].value, state.locale, 0));
    text.append(`tspan`).attr(`dy`, `1.4em`).attr(`x`, `0`).text((key) => getLegendLine(state.cols[key].value, state.locale, 1));
    text.append(`tspan`).attr(`dy`, `1.4em`).attr(`x`, `0`).text((key) => getLegendLine(state.cols[key].value, state.locale, 2));
  };
}
function getLegendLine(value, locale2, index) {
  const [alfa, beta, gama, delta] = value.split(/\.|\s|\.\s|\.&nbsp;|&nbsp;/).filter(Boolean);
  const isDate = [alfa, beta, gama].every((i) => parseInt(i, 10));
  if (isDate)
    return (locale2 === `cs` ? [`${alfa}. ${beta}.`, gama, delta] : [`${alfa}.${beta}.`, gama, delta])[index];
  return [alfa, beta, gama, delta][index];
}
function createYAxis(set2, state) {
  setFormatLocale(state.locale);
  if (isDoughnutChart(state.chartType))
    return () => null;
  const scale = createYScale(set2, state);
  const chartWidth = getChartWidth(state);
  return (g) => {
    const axis2 = axisRight(scale).tickSize(chartWidth).tickPadding(0);
    g.call(axis2);
    g.select(`.domain`).remove();
    g.selectAll(`.tick line`).attr(`opacity`, (d) => d > Math.max(...set2.map(([, val]) => val)) ? 0 : 1).attr(`stroke`, colors.tick);
    g.selectAll(`.tick text`).text((d) => format(`,`)(d)).attr(`opacity`, (d) => d > Math.max(...set2.map(([, val]) => val)) ? 0 : 1).attr(`font-size`, fontSize).attr(`font-family`, fontFamilyPrimary).attr(`fill`, colors.label).attr(`x`, -10).attr(`dy`, 6).attr(`text-anchor`, `end`);
  };
}
function createXScale(set2, state) {
  return band().domain(set2.map(([key]) => key)).rangeRound([0, getChartWidth(state)]).padding(isBarChart(state.chartType) ? 0.33 : 0);
}
function createYScale(set2, state) {
  const { rows, chartType } = state;
  const allValues = Object.values(rows).reduce((acc, curr) => acc.concat(Object.values(curr.values)), []).filter(isNumeric);
  let ratio = 1;
  if (isBarChart(chartType)) {
    const xScale = createXScale(set2, state);
  ...
  return linear().domain([lowerBound, upperBound]).range([getChartHeight(state), 0]);
}
function renderChart(set2, state, chart) {
  setFormatLocale(state.locale);
  ...
  const settings = {
    ...
    xScale: createXScale(set2, state),
    yScale: createYScale(set2, state),
    colors: state.colors,
    cols: state.cols,
    unit: state.unit,
    locale: state.locale,
...
function renderHeader({ title: titleText, subtitle: subtitleText }, element) {
...
function sortData(sortType, sortDirection, cols) {
...
function createTextObject([, number2]) {
  ...
  return { value, unit: unit2, sign: sign2 };
}
...
const barSymbolCommands = svgPathParser.makeAbsolute(svgPathParser.parseSVG(barSymbol));
function createBarSymbol(x2, y2, barWidth) {
  const modifyX = (pointX) => pointX * barWidth / 10 + x2;
  const modifyY = (pointY) => pointY * barWidth / 10 + y2 - barWidth * symbolRatio;
  return modifySVGCommands(barSymbolCommands, modifyX, modifyY).join(``);
}
function createBarPath(x2, y2, barWidth, chartHeight) {
  return [
    `M${x2} ${y2 - barWidth * symbolRatio / 2}`,
    `L${x2} ${chartHeight}`,
    `L${x2 + barWidth} ${chartHeight}`,
    `L${x2 + barWidth} ${y2 - barWidth * symbolRatio / 2}`,
    `Z`
  ].join(` `);
}
function modifySVGCommands(commands, xModificator, yModificator) {
...
function getSymbolCenter(width) {
  return width * symbolRatio / 2;
}
function createLabels(chart, labels, width, height, maxValueLength) {
  const verticalLabel = chart.selectAll(`.verticalLabel`).data([1]);
  const verticalMove = -1 * (maxValueLength + 1) * fontSize * fontAspectRatio;
  const verticalLabelEnter = verticalLabel.enter().append(`text`).attr(`class`, `verticalLabel`).attr(`x`, verticalMove).attr(`transform`, `rotate(-90, ${verticalMove}, ${height})`).attr(`y`, height).attr(`font-family`, fontFamilyPrimary).attr(`fill`, colors.label).attr(`fill-opacity`, 0);
  verticalLabelEnter.merge(verticalLabel).text(labels.verticalLabel).transition().duration(1e3).attr(`fill-opacity`, 1);
  const topLeftLabel = chart.selectAll(`.topLeftLabel`).data([1]);
  topLeftLabel.enter().append(`text`).attr(`class`, `topLeftLabel`).attr(`x`, 0).attr(`y`, 0).attr(`font-family`, fontFamilyPrimary).attr(`fill`, colors.label);
  const topRightLabel = chart.selectAll(`.topRightLabel`).data([1]);
  topRightLabel.enter().append(`text`).attr(`class`, `topRightLabel`).attr(`x`, width - 20).attr(`y`, 0).attr(`font-family`, fontFamilyPrimary).attr(`fill`, colors.label).attr(`fill-opacity`, 1).attr(`text-anchor`, `end`);
  const bottomRightLabel = chart.selectAll(`.bottomRightLabel`).data([1]);
  const bottomRightLabelEnter = bottomRightLabel.enter().append(`text`).attr(`class`, `bottomRightLabel`).attr(`x`, width - 20).attr(`y`, height + labelOffset * 1.75).attr(`font-family`, fontFamilyPrimary).attr(`fill`, colors.label).attr(`text-anchor`, `end`);
  bottomRightLabelEnter.merge(bottomRightLabel).text(labels.horizontalLabel).transition().duration(1e3).attr(`fill-opacity`, 0.5);
}
function setFormatLocale(locale2) {
  const localeMap = {
    [LOCALE_CZECH]: {
      decimal: `,`,
      thousands: ` `,
  ...
  return defaultLocale(localeMap[locale2] || localeMap[LOCALE_CZECH]);
}
function draw(element, state) {
  if (isChart(state.chartType)) {
    state.height = isDoughnutChart(state.chartType) ? state.width : state.height;
    const root2 = select(element).attr(`class`, `renderer ${state.chartType}-chart ${state.orientation}`);
    const chart = prepareChart(root2, state);
    const set2 = getChartSpecificData(state);
