...
export function getChartLeftMargin({ rows, chartType, verticalLabel }) {
  ...
  return (
    getMaxLength(rows) * fontSize * fontAspectRatio +
...
export function getChartSpecificData({ chartType, rows }, forScales = true) {
  if (isLineChart(chartType)) {
    if (!forScales) return pairs(rows);
    const maxValues = {};
...
export function createYAxis(set, state) {
  ...
  return (g) => {
    ...
    g.selectAll(`.tick line`)
      .attr(`opacity`, (d) =>
        d > Math.max(...set.map(([, val]) => val)) ? 0 : 1
      )
...
export function renderChart(set, state, chart) {
  setFormatLocale(state.locale);
  const { chartType, horizontalLabel, verticalLabel, sortDirection, sortType } =
    state;
  const settings = {
  ...
  if (isBarChart(chartType)) return redrawBarChart(settings);
  if (isLineChart(chartType)) return redrawLineChart(settings);
  if (isDoughnutChart(chartType)) return redrawDoughnutChart(settings);
...
export function sortData(sortType, sortDirection, cols) {
  ...
  return ([keyA, valA], [keyB, valB]) => {
    if (sortType === SORT_TYPE_ORIGINAL) return isDesc ? -1 : 1;
    if (sortType === SORT_TYPE_VALUE) return (valA - valB) * (isDesc ? -1 : 1);
...
export function renderChartFooter(
  ...
  if (displayLegend === false) return false;
  ...
  set
    ...
    .forEach(([colKey, value]) => {
      ...
      valueElem.innerHTML = `${valueFormat(value)}${
